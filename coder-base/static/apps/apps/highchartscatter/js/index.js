...
var filename='/mnt/data/itpTemprature.csv';
var removefilename='';
var mdata=''; // 読み込んだデータ
var minValue = '-10';
var maxValue = '40';
...
//console.log('['+i+']'+ary[i]);
        if( isFinite(ary[i]) && !isNaN(ary[i]) ){
            if(datalabel=='temp'  && ary[i] > 40 )   ary[i]=40;
            if(datalabel=='water' && ary[i] > 400  ) ary[i]=400;
            if(datalabel=='illum' && ary[i] > 4000 ) ary[i]=4000;
...
//    if(arx[i] && ary[i] && arx[i] !==arx[i+1] ){
 //console.log(i+' '+arx[i].length+' '+arx[i]+' '+ary[i]);       
    point.addPoint([Date.UTC(arx[i].substr(0,4), arx[i].substr(4,2)-1, arx[i].substr(6,2),arx[i].substr(8,2),arx[i].substr(10,2),arx[i].substr(10,2)),parseFloat(ary[i])]);
